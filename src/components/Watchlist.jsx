...
const Watchlist = () => {
        ...
        const enrichedWatchlist = await Promise.all(
          watchlistData.map(async (entry) => {
            try {
              ...
              console.error(`Error fetching details for movie ID ${entry.tmdbMovieId}:`, movieError);
              return {
                ...entry,
                title: 'Unknown Title',
  ...
  const handleDelete = async () => {
    ...
    const token = localStorage.getItem('token');
    try {
      await axios.delete(`http://localhost:8080/api/watchlist/${movieToDelete.watchlistId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setWatchlist(watchlist.filter((movie) => movie.watchlistId !== movieToDelete.watchlistId));
      ...
      if (error.response && error.response.status === 401) {
        ...
        alert('An error occurred while removing the movie from your watchlist.');
      }
    }
  };
          ...
          {watchlist.map((movie) => (
            <div key={movie.watchlistId} className="col-md-4 mb-4">
              <div className="card movie-card">
                <Link to={`/movie/${movie.tmdbMovieId}`} className="text-decoration-none">
                  <img
                    src={
                      movie.posterPath
                  ...
                  <div className="card-body">
                    ...
                    <p className="card-text">Added on: {new Date(movie.addedAt).toLocaleDateString()}</p>
                  </div>
                </Link>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <p>Are you sure you want to remove "{movieToDelete?.title || 'this movie'}" from your watchlist?</p>
              </div>
